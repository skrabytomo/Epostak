program EpostakTestGUI;

{ ============================================================================
  To iste ako EpostakTest.dpr, ale vo forme - okno neprekvapi zatvorenim,
  log ostava viditelny a scrollovatelny. Ziadny .dfm, cely formular je
  vytvoreny v kode (FormCreate) - bezpecne, nic sa neda poskodit.
  ============================================================================ }

uses
  Forms, Windows, Messages, SysUtils, Classes, Controls, StdCtrls,
...
begin
  Memo.Lines.Add(AMsg);
  { vynuti prekreslenie a scroll na koniec, aj ked bezi dlha sync operacia }
...
begin
  ...
  FormTest := TFormTest.CreateNew(nil);
  FormTest.Caption := 'Epostak SAPI-SK Test';
    FormTest.Width := 800;
    FormTest.Height := 500;
    FormTest.Position := poScreenCenter;

    FormTest.BtnRun := TButton.Create(FormTest);
    FormTest.BtnRun.Parent := FormTest;
    FormTest.BtnRun.Caption := 'Spustit test';
    FormTest.BtnRun.SetBounds(10, 10, 120, 30);
    FormTest.BtnRun.OnClick := FormTest.BtnRunClick;

    FormTest.Memo := TMemo.Create(FormTest);
    FormTest.Memo.Parent := FormTest;
    FormTest.Memo.SetBounds(10, 50, 770, 400);
    FormTest.Memo.ScrollBars := ssVertical;
    FormTest.Memo.Font.Name := 'Consolas';
    FormTest.Memo.Font.Size := 9;
    FormTest.Memo.ReadOnly := True;
    FormTest.Memo.WordWrap := False;
    FormTest.Memo.Anchors := [akLeft, akTop, akRight, akBottom];
    FormTest.BtnRun.Anchors := [akLeft, akTop];
